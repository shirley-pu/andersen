...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZBINU,ZABS2,I1MACH,D1MACH
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZACAI,ZBKNU,ZEXP2,ZSQRT2,ZABS2,I1MACH,D1MACH
...
C-----------------------------------------------------------------------
   ...
   40 CONTINUE
      ...
      IF (KODE.EQ.1) RETURN
      CALL ZSQRT2(ZR, ZI, STR, STI)
      ZTAR = TTH*(ZR*STR-ZI*STI)
      ZTAI = TTH*(ZR*STI+ZI*STR)
      CALL ZEXP2(ZTAR, ZTAI, STR, STI)
      PTR = AIR*STR - AII*STI
      AII = AIR*STI + AII*STR
      AIR = PTR
      RETURN
...
C                 PP 265-273.
C
C***ROUTINES CALLED  ZBINU,ZABS2,ZDIV2,ZSQRT2,D1MACH,I1MACH
...
C-----------------------------------------------------------------------
      CALL ZDIV2(CYR(1), CYI(1), ZTAR, ZTAI, STR, STI)
...
C***END PROLOGUE  ZMLT2
      ...
      RETURN
      END
      SUBROUTINE ZDIV2(AR, AI, BR, BI, CR, CI)
C***BEGIN PROLOGUE  ZDIV2
C***REFER TO  ZBESH,ZBESI,ZBESJ,ZBESK,ZBESY,ZAIRY,ZBIRY
C
C     DOUBLE PRECISION COMPLEX DIVIDE C=A/B.
C
C***ROUTINES CALLED  ZABS2
C***END PROLOGUE  ZDIV2
      EXTERNAL ZABS2
      ...
      RETURN
      END
      SUBROUTINE ZSQRT2(AR, AI, BR, BI)
C***BEGIN PROLOGUE  ZSQRT2
C***REFER TO  ZBESH,ZBESI,ZBESJ,ZBESK,ZBESY,ZAIRY,ZBIRY
C
C     DOUBLE PRECISION COMPLEX SQUARE ROOT, B=CSQRT(A)
C
C***ROUTINES CALLED  ZABS2
C***END PROLOGUE  ZSQRT2
      EXTERNAL ZABS2
   ...
   70 BR = ZM*DRT
      ...
      RETURN
      END
      SUBROUTINE ZEXP2(AR, AI, BR, BI)
C***BEGIN PROLOGUE  ZEXP2
C***REFER TO  ZBESH,ZBESI,ZBESJ,ZBESK,ZBESY,ZAIRY,ZBIRY
C
C     DOUBLE PRECISION COMPLEX EXPONENTIAL FUNCTION B=EXP(A)
C
C***ROUTINES CALLED  (NONE)
C***END PROLOGUE  ZEXP2
...
C
C***ROUTINES CALLED  DGAMLN,I1MACH,D1MACH,ZKSCL,ZSHCH2,ZUCHK,ZABS2,ZDIV2,
C                    ZEXP2,ZLOG2,ZMLT2,ZSQRT2
...
C-----------------------------------------------------------------------
      FC = 1.0D0
      CALL ZLOG2(RZR, RZI, SMUR, SMUI, IDUM)
      FMUR = SMUR*DNU
      FMUI = SMUI*DNU
      CALL ZSHCH2(FMUR, FMUI, CSHR, CSHI, CCHR, CCHI)
...
C-----------------------------------------------------------------------
   ...
   50 CONTINUE
      ...
      CALL ZEXP2(FMUR, FMUI, STR, STI)
      PR = 0.5D0*STR/T2
      PI = 0.5D0*STI/T2
      CALL ZDIV2(0.5D0, 0.0D0, STR, STI, PTR, PTI)
      QR = PTR/T1
      QI = PTI/T1
      S1R = FR
      S1I = FI
...
C-----------------------------------------------------------------------
      ...
      CALL ZMLT2(ZR, ZI, ZR, ZI, CZR, CZI)
      CZR = 0.25D0*CZR
      CZI = 0.25D0*CZI
      T1 = 0.25D0*CAZ*CAZ
   60 CONTINUE
   ...
   70 CONTINUE
      ...
      CALL ZEXP2(ZR, ZI, STR, STI)
      CALL ZMLT2(S1R, S1I, STR, STI, YR(1), YI(1))
      RETURN
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
  ...
  100 CONTINUE
      ...
      P2R = S2R*STR
      P2I = S2I*STR
      CALL ZMLT2(P2R, P2I, RZR, RZI, S2R, S2I)
      S1R = S1R*STR
      S1I = S1I*STR
      IF (KODED.EQ.1) GO TO 210
      CALL ZEXP2(ZR, ZI, FR, FI)
      CALL ZMLT2(S1R, S1I, FR, FI, S1R, S1I)
      CALL ZMLT2(S2R, S2I, FR, FI, S2R, S2I)
      GO TO 210
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
  110 CONTINUE
      CALL ZSQRT2(ZR, ZI, STR, STI)
      CALL ZDIV2(RTHPI, CZEROI, STR, STI, COEFR, COEFI)
      KFLAG = 2
...
C     BLANK LINE
      ...
      STI = -STR*DSIN(ZI)
      STR = STR*DCOS(ZI)
      CALL ZMLT2(COEFR, COEFI, STR, STI, COEFR, COEFI)
  120 CONTINUE
...
C-----------------------------------------------------------------------
      ...
      CSR = CSR*PTR
      CSI = -CSI*PTR
      CALL ZMLT2(COEFR, COEFI, S1R, S1I, STR, STI)
      CALL ZMLT2(STR, STI, CSR, CSI, S1R, S1I)
...
C-----------------------------------------------------------------------
      ...
      P2R = P2R*PTR
      P2I = -P2I*PTR
      CALL ZMLT2(P1R, P1I, P2R, P2I, PTR, PTI)
      STR = DNU + 0.5D0 - PTR
      STI = -PTI
      CALL ZDIV2(STR, STI, ZR, ZI, STR, STI)
      STR = STR + 1.0D0
      CALL ZMLT2(STR, STI, S1R, S1I, S2R, S2I)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
  261 CONTINUE
      ...
      DO 262 I=1,INU
        ...
        CALL ZLOG2(S2R,S2I,STR,STI,IDUM)
        P2R = -ZDR+STR
        P2I = -ZDI+STI
        P2M = DEXP(P2R)/TOL
        P1R = P2M*DCOS(P2I)
...
C
C***ROUTINES CALLED  ZUCHK,ZABS2,ZLOG2
...
C
      ...
      DO 10 I=1,NN
        ...
        CALL ZLOG2(S1R, S1I, CSR, CSI, IDUM)
        CSR = CSR - ZRR
        CSI = CSI - ZRI
        STR = DEXP(CSR)/TOL
        CSR = STR*DCOS(CSI)
...
C
      DO 30 I=3,N
        ...
        CALL ZLOG2(S2R, S2I, CSR, CSI, IDUM)
        CSR = CSR - ZDR
        CSI = CSI - ZDI
        STR = DEXP(CSR)/TOL
        CSR = STR*DCOS(CSI)
   ...
   50 CONTINUE
      RETURN
      END
      SUBROUTINE ZSHCH2(ZR, ZI, CSHR, CSHI, CCHR, CCHI)
C***BEGIN PROLOGUE  ZSHCH2
C***REFER TO  ZBESK,ZBESH
C
C     ZSHCH2 COMPUTES THE COMPLEX HYPERBOLIC FUNCTIONS CSH=SINH(X+I*Y)
C     AND CCH=COSH(X+I*Y), WHERE I**2=-1.
C
C***ROUTINES CALLED  (NONE)
C***END PROLOGUE  ZSHCH2
C
...
C-----------------------------------------------------------------------
   ...
   30 CONTINUE
      ...
      P1I = TOL
   40 CONTINUE
      CALL ZDIV2(P2R, P2I, P1R, P1I, CYR(N), CYI(N))
      IF (N.EQ.1) RETURN
      K = N - 1
      AK = DBLE(FLOAT(K))
      T1R = AK
...
C
C***ROUTINES CALLED  ZABS2,ZEXP2,ZLOG2
...
C     COMPLEX CZERO,C1,S1,S1D,S2,ZR
     ...
     * S1DR, S1I, S1R, S2I, S2R, ZEROI, ZEROR, ZRI, ZRR, ZABS2
      ...
      CALL ZLOG2(S1DR, S1DI, C1R, C1I, IDUM)
      C1R = C1R - ZRR - ZRR
      C1I = C1I - ZRI - ZRI
      CALL ZEXP2(C1R, C1I, S1R, S1I)
      AS1 = ZABS2(S1R,S1I)
      IUF = IUF + 1
   10 CONTINUE
      AA = DMAX1(AS1,AS2)
...
C
C***ROUTINES CALLED  DGAMLN,D1MACH,ZABS2,ZEXP2,ZLOG2,ZMLT2
...
C-----------------------------------------------------------------------
   ...
   90 CONTINUE
      ...
      CALL ZLOG2(RZR, RZI, STR, STI, IDUM)
      P1R = -FNF*STR + PTR
      P1I = -FNF*STI + PTI
...
C-----------------------------------------------------------------------
      ...
      AP = ZABS2(P2R,P2I)
      P1R = 1.0D0/AP
      CALL ZEXP2(PTR, PTI, STR, STI)
      CKR = STR*P1R
      CKI = STI*P1R
      PTR = P2R*P1R
      PTI = -P2I*P1R
      CALL ZMLT2(CKR, CKI, PTR, PTI, CNORMR, CNORMI)
      DO 100 I=1,N
...
C
C***ROUTINES CALLED  DGAMLN,D1MACH,ZUCHK,ZABS2,ZDIV2,ZLOG2,ZMLT2
...
C
      ...
      CALL ZMLT2(HZR, HZI, HZR, HZI, CZR, CZI)
   10 CONTINUE
      ACZ = ZABS2(CZR,CZI)
      NN = N
      CALL ZLOG2(HZR, HZI, CKR, CKI, IDUM)
   20 CONTINUE
...
C-----------------------------------------------------------------------
   ...
   80   CONTINUE
        ...
        YI(M) = S2I*CRSCR
        IF (I.EQ.IL) GO TO 90
        CALL ZDIV2(COEFR, COEFI, HZR, HZI, STR, STI)
        COEFR = STR*DFNU
        COEFI = STI*DFNU
   90 CONTINUE
      IF (NN.LE.2) RETURN
...
C
C***ROUTINES CALLED  D1MACH,ZABS2,ZDIV2,ZEXP2,ZMLT2,ZSQRT2
...
C-----------------------------------------------------------------------
      ...
      AK1R = RTPI*STR*RAZ
      AK1I = RTPI*STI*RAZ
      CALL ZSQRT2(AK1R, AK1I, AK1R, AK1I)
      CZR = ZR
      CZI = ZI
   ...
   10 CONTINUE
      ...
      KODED = 0
      CALL ZEXP2(CZR, CZI, STR, STI)
      CALL ZMLT2(AK1R, AK1I, STR, STI, AK1R, AK1I)
   20 CONTINUE
      FDN = 0.0D0
...
C-----------------------------------------------------------------------
   ...
   30 CONTINUE
      DO 70 K=1,IL
        ...
        DKI = EZI
        DO 40 J=1,JL
          CALL ZDIV2(CKR, CKI, DKR, DKI, STR, STI)
          CKR = STR*SQK
          CKI = STI*SQK
          CS2R = CS2R + CKR
          CS2I = CS2I + CKI
   ...
   50   CONTINUE
        ...
        TZR = ZR + ZR
        TZI = ZI + ZI
        CALL ZEXP2(-TZR, -TZI, STR, STI)
        CALL ZMLT2(STR, STI, P1R, P1I, STR, STI)
        CALL ZMLT2(STR, STI, CS2R, CS2I, STR, STI)
        S2R = S2R + STR
        S2I = S2I + STI
   60   CONTINUE
...
C             ANOTHER ROUTINE
C
C***ROUTINES CALLED  ZUCHK,ZUNHJ,ZUNIK,D1MACH,ZABS2,ZLOG2
...
C-----------------------------------------------------------------------
  ...
  110 CONTINUE
      ASCLE = 1.0D+3*D1MACH(1)/TOL
      CALL ZLOG2(PHIR, PHII, STR, STI, IDUM)
      CZR = CZR + STR
      CZI = CZI + STI
      IF (IFORM.EQ.1) GO TO 120
      CALL ZLOG2(ARGR, ARGI, STR, STI, IDUM)
...
C     HALF Z PLANE
C
C***ROUTINES CALLED  ZBINU,ZBKNU,ZS1S2,D1MACH,ZABS2,ZMLT2
...
C-----------------------------------------------------------------------
      ...
      CPN = DCOS(YY)
      SPN = DSIN(YY)
      CALL ZMLT2(CSGNR, CSGNI, CPN, SPN, CSGNR, CSGNI)
   10 CONTINUE
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
   ...
   20 CONTINUE
      ...
      SC1I = C1I
   30 CONTINUE
      CALL ZMLT2(CSPNR, CSPNI, C1R, C1I, STR, STI)
      CALL ZMLT2(CSGNR, CSGNI, C2R, C2I, PTR, PTI)
      YR(1) = STR + PTR
      YI(1) = STI + PTI
      IF (N.EQ.1) RETURN
      CSPNR = -CSPNR
...
C        ZETA1,ZETA2.
C
C***ROUTINES CALLED  ZDIV2,ZLOG2,ZSQRT2,D1MACH
...
C-----------------------------------------------------------------------
   ...
   15 CONTINUE
      ...
      CALL ZSQRT2(SR, SI, SRR, SRI)
      STR = CONER + SRR
      STI = CONEI + SRI
      CALL ZDIV2(STR, STI, TR, TI, ZNR, ZNI)
      CALL ZLOG2(ZNR, ZNI, STR, STI, IDUM)
      ZETA1R = FNU*STR
      ZETA1I = FNU*STI
      ZETA2R = FNU*SRR
      ZETA2I = FNU*SRI
      CALL ZDIV2(CONER, CONEI, SRR, SRI, TR, TI)
      SRR = TR*RFN
      SRI = TI*RFN
      CALL ZSQRT2(SRR, SRI, CWRKR(16), CWRKI(16))
      PHIR = CWRKR(16)*CON(IKFLG)
      PHII = CWRKI(16)*CON(IKFLG)
      IF (IPMTR.NE.0) RETURN
      CALL ZDIV2(CONER, CONEI, SR, SI, T2R, T2I)
      CWRKR(1) = CONER
      CWRKI(1) = CONEI
      CRFNR = CONER
      CRFNI = CONEI
...
C
C***ROUTINES CALLED  ZABS2,ZDIV2,ZLOG2,ZSQRT2,D1MACH
...
C-----------------------------------------------------------------------
   ...
   20 CONTINUE
      ...
      ARGR = ZETAR*FN23
      ARGI = ZETAI*FN23
      CALL ZSQRT2(SUMAR, SUMAI, ZAR, ZAI)
      CALL ZSQRT2(W2R, W2I, STR, STI)
      ZETA2R = STR*FNU
      ZETA2I = STI*FNU
...
C-----------------------------------------------------------------------
  130 CONTINUE
      CALL ZSQRT2(W2R, W2I, WR, WI)
      IF (WR.LT.0.0D0) WR = 0.0D0
      IF (WI.LT.0.0D0) WI = 0.0D0
      STR = CONER + WR
      STI = WI
      CALL ZDIV2(STR, STI, ZBR, ZBI, ZAR, ZAI)
      CALL ZLOG2(ZAR, ZAI, ZCR, ZCI, IDUM)
  ...
  140 CONTINUE
      ...
      ARGR = ZETAR*FN23
      ARGI = ZETAI*FN23
      CALL ZDIV2(ZTHR, ZTHI, ZETAR, ZETAI, RTZTR, RTZTI)
      CALL ZDIV2(RTZTR, RTZTI, WR, WI, ZAR, ZAI)
      TZAR = ZAR + ZAR
      TZAI = ZAI + ZAI
      CALL ZSQRT2(TZAR, TZAI, STR, STI)
      PHIR = STR*RFN13
      PHII = STI*RFN13
      IF (IPMTR.EQ.1) GO TO 120
      RAW = 1.0D0/DSQRT(AW2)
...
C-----------------------------------------------------------------------
  ...
  220 CONTINUE
      ...
      STR = -BSUMR*RFN13
      STI = -BSUMI*RFN13
      CALL ZDIV2(STR, STI, RTZTR, RTZTI, BSUMR, BSUMI)
      GO TO 120
      END
